...
public class StockMarketServiceTest {
	...
	/**
	 * Tests for {@link StockMarketService#registerStock(AbstractStock)} and
	 * {@link StockMarketService#unregisterStock(String)}.
	 * <ul>
	 * <li>call the {@link StockMarketService#registerStock(AbstractStock)}
	 ...
	 * <ul>
	 * <li>record a TEST_COMMON_STOCK trade in 3 minutes ago which bought 123456 shares at the price 1235</li>
	 * <li>record a TEST_COMMON_STOCK trade in 12 minutes ago which sold 7890 shares at the price 1021</li>
	 * <li>record a TEST_PREFERRED_STOCK trade in 7 minutes ago which bought 10000 shares at the price 3012</li>
	 * <li>record a TEST_COMMON_STOCK trade in 16 minutes ago which bought 2400 shares at the price 1187</li>
	 * <li>call the {@link StockMarketService#recordTrade(String, Date,
	 * BigInteger, BuyOrSell, BigDecimal)</li>
	...
	public void testVolumWeightedStockPrice(){
		...
		this.service.recordTrade(TestUtils.TEST_COMMON_STOCK, new Date(startTime-3*1000*60), new BigInteger("123456"), BuyOrSell.BUY, new BigDecimal(1235));
		this.service.recordTrade(TestUtils.TEST_COMMON_STOCK, new Date(startTime-12*1000*60), new BigInteger("7890"), BuyOrSell.SELL, new BigDecimal(1021));
		this.service.recordTrade(TestUtils.TEST_PREFERRED_STOCK, new Date(startTime-7*1000*60), new BigInteger("10000"), BuyOrSell.BUY, new BigDecimal(3012));
		this.service.recordTrade(TestUtils.TEST_COMMON_STOCK, new Date(startTime-16*1000*60), new BigInteger("2400"), BuyOrSell.BUY, new BigDecimal(1187));
		Assert.assertEquals(new BigDecimal(1222), this.service.getVolumeWeightedStockPrice(TestUtils.TEST_COMMON_STOCK));
	}
	
	@Test
	public void testGBCEAllShareIndex(){
		CommonStock commonStock1 = new CommonStock(TestUtils.TEST_COMMON_STOCK+1, BigDecimal.ZERO,BigDecimal.ZERO,new BigDecimal(1234));
		CommonStock commonStock2 = new CommonStock(TestUtils.TEST_COMMON_STOCK+2, BigDecimal.ZERO,BigDecimal.ZERO,new BigDecimal(7057));
		CommonStock commonStock3 = new CommonStock(TestUtils.TEST_COMMON_STOCK+4, BigDecimal.ZERO,BigDecimal.ZERO,new BigDecimal(4069));
		PreferredStock preferStock1 = new PreferredStock(TestUtils.TEST_PREFERRED_STOCK+1, BigDecimal.ZERO,BigDecimal.ZERO,new BigDecimal(583), BigDecimal.ZERO);
		PreferredStock preferStock2 = new PreferredStock(TestUtils.TEST_PREFERRED_STOCK+2, BigDecimal.ZERO,BigDecimal.ZERO,new BigDecimal(11231), BigDecimal.ZERO);
